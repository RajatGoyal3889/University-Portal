...
{
Statement st1=c.createStatement();
ResultSet rs1=st1.executeQuery("SELECT Colleges.id, Colleges.CollegeName, Colldetail.addd, Colldetail.ph, Colldetail.fax, Colldetail.pri, Colldetail.priph, Colldetail.mail, Login.Name, Login.password FROM (Colleges INNER JOIN Colldetail ON Colleges.id = Colldetail.id) INNER JOIN Login ON Colleges.id = Login.id where Colleges.CollegeName='"+coll+"'");
while(rs1.next())
{
id=rs1.getString(1);
...
out.println("<table bordercolor='' border='1' bgcolor=''><form name='form1' action='Admin3.jsp'><tr><td></div></p><p><div align='right'>COLLEGE NAME:&nbsp;&nbsp;<input type='text' name='coll' value="+collegeold+"></div></p><p><div align='right'>PLACED AT: &nbsp;&nbsp;<input type='text' name='place' value="+place+"></div></p><p><div align='right'>COLLEGE CONTACT NO: &nbsp;&nbsp;<input type='text' name='cph' value="+cph+"></div></p><p><div align='right'>FAX NO: &nbsp;&nbsp;<input type='text' name='fax' value="+fax+"></div></p><p><div align='right'>PRINCIPAL NAME:&nbsp;&nbsp;&nbsp;<input type='text' name='princ' value="+princ+"></div></p><p><div align='right'>PHONE NO: &nbsp;&nbsp;&nbsp;<input type='text' name='pno' value="+pno+"></div></p><p><div align='right'>MAIL ID:&nbsp;&nbsp;&nbsp; <input type='text' name='mail' value="+mail+"></div></p><p><div align='right'>INITIAL LOGIN ID:&nbsp;&nbsp;&nbsp; <input type='text' name='login' value="+login+"></div></p><p><div align='right'>INITIAL PASSWORD:&nbsp;&nbsp;&nbsp; <input type='text' name='pass' value="+pass+"></div></p><p><div align='center'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input  type='submit' name='name' value= 'CONFIRM' onclick='return validationTxt()'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input  type='submit' name='name' value= 'Home' ></p></div></td></tr></form></TABLE>");
}
...
PreparedStatement ps3=c.prepareStatement("update Colldetail set addd=?,ph=?,fax=?,pri=?,priph=?,mail=? where id=?");
